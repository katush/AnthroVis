...
\begin{figure*}[tbh]
	\centering
  \includegraphics[width=1\linewidth]{pictures/pipeline.pdf}
...
Traditional applications for mesh processing and comparison offer a large variety of algorithms for alignment and comparison, but are lacking in visual representation of results, relying in most cases only on color maps.
These, although considered as advanced visualization tools in forensic and biological sciences (e.g., \cite{urbanova2015testing}), have very limited possibilities, particularly while comparing large datasets of 3D facial meshes.
On the other hand, visual analysis tools targeting mesh comparison often fail in supporting domain-specific tasks required by anthropologists.
AnthroVis tool aims to overcome these problems by integrating interactively linked visualization techniques into an analysis workflow tailored to the needs of domain experts.
